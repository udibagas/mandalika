<template>
    <el-card header="KELOLA USER">
        ...
        v-loading="loading">
            ...
            <el-table-column prop="active" label="Status" align="center" header-align="center" width="120px">
                <template slot-scope="scope">
                    <el-tag effect="dark" size="small" style="width:100%" :type="scope.row.status ? 'success' : 'danger'">{{scope.row.status ? 'Aktif' : 'Nonaktif'}}</el-tag>
                </template>
            </el-table-column>
...
export default {
    ...
    methods: {
        ...
        requestData() {
            ...
            axios.get('/user', { params }).then(r => this.tableData = r.data)
                .catch(e => {
                    if (e.response.status == 500) {
                        this.$message({
